...
void mover_servo(int8_t pos)
//void mover_servo_f()
{
  if (posicionado == pos)
    return; // ya esta posicionado, no lo muevo.
  posicionado = pos;
  servo_position = servo_positions[pos];
...
{
  ADMUX |= (1 << ADLAR) | (1 << MUX0) | (1 << MUX1);   // left adjust result, Set the ADC input
